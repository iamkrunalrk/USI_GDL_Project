...
if __name__ == "__main__":
    ...
    with open(Path(os.path.join(arg.bone_dir, 'epoch1_test_score.pkl')), 'rb') as r2:
        r2 = list(pickle.load(r2).items())

    right_num = total_num = right_num_5 = 0
    for i in tqdm(range(len(r1))): #Changed from:     for i in tqdm(range(len(label[0]))):
        print(i, len(label[0]), len(r1))
        _, l = label[:, i]
        _, r11 = r1[i]
        _, r22 = r2[i]
        r = r11 + r22 * arg.alpha
